...
print "The Results of Simulation III"
...
for au in AuthorMatchTopics:
    if entropyOld[au]<=entropyNew[au]:
        entropyTot += 1
        if (entropyNew[au]<=entropyNew2[au]) and (entropyNew[au]-entropyOld[au] >= entropyNew2[au]-entropyNew[au]):  # Marginally decrease
            entropyPro += 1
    if entropy2Old[au]<=entropyNew[au]: 
        entropy2Tot += 1
        if (entropy2New[au]<=entropy2New2[au]) and (entropy2New[au]-entropy2Old[au] >= entropy2New2[au] - entropy2New[au]): #Marginally decrease
            entropy2Pro += 1
    if SimpOld[au]<=SimpNew[au]: 
        SimpTot += 1
        if (SimpNew[au]<=SimpNew2[au]) and (SimpNew[au]-SimpOld[au] >= SimpNew2[au]-SimpNew[au]): # Marginally decrease
            SimpPro += 1
    if GiniOld[au]<=GiniNew[au]:
        GiniTot += 1
        if (GiniNew[au]<=GiniNew2[au]) and (GiniNew[au]-GiniOld[au] >= GiniNew2[au]-GiniNew[au]): # Marginally decrease
            GiniPro += 1
    if GLscoreOld[au]<=GLscoreNew[au]: 
        GLscoreTot += 1
        if (GLscoreNew[au]<=GLscoreNew2[au]) and (GLscoreNew[au]-GLscoreOld[au] >= GLscoreNew2[au] - GLscoreNew[au]): # Marginally decrease
            GLscorePro += 1
    if Shiyan1Old[au]<=Shiyan1New[au]: 
        Shiyan1Tot += 1
        if (Shiyan1New[au]<=Shiyan1New2[au]) and (Shiyan1New[au]-Shiyan1Old[au] >= Shiyan1New2[au] - Shiyan1New[au]): # Marignally decrease 
            Shiyan1Pro += 1
